...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    pool1_out, pool1_cache = max_pool_forward_naive(conv1_out, pool_param)
    affine1_out, affine1_cache = affine_relu_forward(pool1_out, W2, b2)
    ...
    loss += 0.5*self.reg*(
                          np.sum(np.multiply(W1, W1)) +
                          np.sum(np.multiply(W2, W2)) +
                          np.sum(np.multiply(W3, W3))
                         )
    ...
    # affine_relu 1
    affine1_dx, affine1_dw, affine1_db = affine_relu_backward(affine2_dx, affine1_cache)
    #print('affine1_dw.shape', affine1_dw.shape)
    #print('W2.shape', W2.shape)
    grads['W2'] = affine1_dw + self.reg * self.params['W2']
    grads['b2'] = affine1_db
    
    # pool 1    
    pool1_dx = max_pool_backward_naive(affine1_dx, pool1_cache)
    
    # conv 1
    conv1_dx, conv1_dw, conv1_db = conv_relu_backward(pool1_dx, conv1_cache)
